#!/usr/bin/env python3
"""
Evaluate Spider LoRA adapter on the Spider test set.

Saves predictions to disk before scoring so scoring can be re-run
without re-doing inference via --score-only.

Usage:
    CUDA_VISIBLE_DEVICES=2 python scripts/eval_lora_spider.py
    CUDA_VISIBLE_DEVICES=2 python scripts/eval_lora_spider.py --output results/spider_lora_eval.json

    # Re-score saved predictions without inference:
    python scripts/eval_lora_spider.py --score-only --predictions-dir results/predictions
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate Spider LoRA adapter on Spider test set")
    parser.add_argument("--config", type=str, default="configs/config.yaml")
    parser.add_argument("--output", type=str, default="results/spider_lora_eval.json",
...
def main():
    ...
    print("=" * 60)
    print("  Spider LoRA Adapter Evaluation")
    print("=" * 60)
    ...
    if adapter_path is None:
        ...
        if not adapters:
            print("ERROR: No Spider adapters found in registry!")
            return
    ...
    if args.score_only:
        ...
        # Inference
        print(f"\n{'=' * 60}")
        print(f"  Evaluating Spider LoRA on Spider test ({len(spider_test)} samples)")
        print(f"{'=' * 60}")
    ...
    print(f"\n  Spider LoRA Score: {result.score:.2%} ({result.correct}/{result.num_samples})")
    ...
    print(f"{'=' * 60}")
    print(f"  Spider LoRA:  {result.score:.2%}  ({result.correct}/{result.num_samples})  [{result.metric_name}]")
    print(f"  Spider Base:  72.34%  (748/1034)  [execution_accuracy]  (from previous run)")
    improvement = result.score - 0.7234
    if improvement > 0:
